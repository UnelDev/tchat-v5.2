	  ...
	  ui(new Ui::Widget)
{
    ...
    QObject::connect(clients, &client::externalOrder, this, &Widget::internalCmd);
...
void Widget::internalCmd(const QString cmd){internalcommende(cmd);};
void Widget::internalcommende(QString commende, QVariant arg1, QVariant arg2)
{
	if(commende=="clearForAll"){
		clients->sendcommande("clearForAll");
	}else if(commende=="clear"){
	QLayoutItem* child;
	...
	}else if(commende=="promot"){
		QList<QString> userList;
		for(int i=0;i<ui->clientlist->count();i++){
			userList.append(ui->clientlist->item(i)->text());
		}
		m_chatBot->startChangeUserRoom(userList);
	}else if(commende=="userRoom"){
		QList<QString> userList;
		for(int i=0;i<ui->clientlist->count();i++){
			userList.append(ui->clientlist->item(i)->text());
		}
        m_chatBot->startUserAction(userList);
	}else if(commende=="condensed"){
        condesed();
    }else if(commende=="message"){
        displayMessagelist(arg1.toString());
    }
}
...
void Widget::processechatbot(const QString command, const QString psedo)
{
    m_chatBot->proccess(command, psedo);
}
...
void Widget::on_sentbuton_clicked()
{
	...
	{
		message = message.remove("/");
        processechatbot(message,returnpseudo());
	}
